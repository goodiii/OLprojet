	...
	if (is_array($_POST['test']))
		...
		foreach ($_POST['test'] as $titreref)
			{				
				...
				if ($req = $bdd->query('DELETE FROM projets WHERE titre ="'.$titreref.'"'))
					{
					echo 'Le projet '.$titreref.' a bien été supprimé de la base de données <br />';
					}	
				if (unlink('page/'.$url.'.php'))
					{
					echo 'La page '.$titreref.' a bien ete supprimé <br /> <a href="navolcms.php" style="color:white"><p>Retour au menu principal</p></a><br/>';
					}
			}
		}	
	}
